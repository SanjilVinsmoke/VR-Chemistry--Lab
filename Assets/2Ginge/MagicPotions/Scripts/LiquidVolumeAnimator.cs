...
public class LiquidVolumeAnimator : MonoBehaviour {
    ...
    public float anchorLength = 2.0f;
	...
	void Start ()
	{
	    ...
	    {
            Vector3 wPos = transform.TransformDirection(verts[i]) - cPos;
	...
	void FixedUpdate ()
	{
        ...
        minMaxBounds.x = minMaxBounds.y = (transform.TransformPoint(verts[0]) - cPos).y;
        for (int i = 0; i < verts.Length; ++i)
        {
            Vector3 wPos = transform.TransformPoint(verts[i]) - cPos;
            if (wPos.y > minMaxBounds.y)
            {
                minMaxBounds.y = wPos.y;
            }
	   ...
	   // Vector3 anchorD = cPos - transform.TransformDirection(Vector3.up) * anchorLength;
        ...
        for (int i = 0; i < mats.Length; ++i)
        {
			if (!mats [i].shader.name.Contains ("_Texture"))
				continue;
